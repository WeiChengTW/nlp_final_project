...
class ArticleFilter(object):
    ...
    def process_raw_data(
        self, path, is_dir=False, to_one_file=False, one_file_name="corpus.json"
    ):
        ...
        for filename in filenames:
            ...
            with open(os.path.join(path, filename), "r", encoding="utf-8") as data:
                ...
                if to_one_file:
                    ...
                    with open(
                        "data/processed/" + filename, "w", encoding="utf-8"
                    ) as op:
    ...
    def merge_coprus(self, path="data/processed/"):
        ...
        corpus_names = [
            ...
            if not name.startswith(".") and name != "reply"
        ]
        all_corpus = []
    ...
    def load_processed_corpus(self, path="data/processed/"):
        ...
        corpus_names = [
            ...
            and os.path.isfile(os.path.join("data/processed/", name))
        ]
        ...
        for article in self.corpus:
            if "Title" not in article:
                logging.warning(f"缺少 Title 欄位，文章內容：{article}")
                continue
    ...
    def clean_responses(
        self, responses, negative_user=set(), min_length=6, stopwords=None
    ):
        """
        ...
        for response in responses:
            ...
            # 濾除過短與特定使用者的回應
            if (
                response["User"] in negative_user
                or len(response["Content"]) < min_length
            ):
                drop = True
            # 濾除包含停用詞的回應
            for w in stopwords:
    ...
    def print_titles(self):
        ...
        with open("data/Titles.txt", "w", encoding="utf-8") as op:
            for title in self.order_titles:
                # 只刪除第一個 "]" 或 "] " 前的字
                if "]" in title:
                    op.write(title.split("]", 1)[1].lstrip() + "\n")
                else:
                    op.write(title + "\n")
    ...
    def print_response(self):
        os.makedirs("data/processed/reply/", exist_ok=True)
        ...
        for response in self.order_response:
            ...
            if sc % 1000 == 0:
                with open(
                    "data/processed/reply/" + str(int(sc / 1000) - 1) + ".json",
                    "w",
                    encoding="utf-8",
                ) as tr:
